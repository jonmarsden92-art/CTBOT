...
def analyse_coin(df: pd.DataFrame, agent: dict, regime: str) -> Optional[dict]:
    try:
        ...
        if regime == "crash":
            ...
            buy_score *= 1.8 * rm  # very aggressive
...
def check_exits(api, positions: list, peak_prices: dict,
    ...
    for pos in positions:
        ...
        if regime == "bull":
            dyn_tp    = max(dyn_tp, 0.03)   # 3% TP — bank faster
...
def run_bot():
    ...
    for pair, df in all_bars.items():
        ...
        if regime == "bear":
            ...
            sig_min = max(sig_min, 3)   # very relaxed in bull
