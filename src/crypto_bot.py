...
def check_exits(api, positions: list, peak_prices: dict,
    ...
    for pos in positions:
        ...
        # Signal exit only after 20 min hold AND in profit
        elif sell_score >= thr["signal_min"] * 3.0 and plpc > 0.01 and hold_mins >= 20:
            reason = "signal_exit"
            log.info("📊 SIGNAL EXIT " + symbol + " +" + str(round(plpc * 100, 1)) + "%")
        elif hold_mins < 20 and hold_mins > 0:
            log.info("⏳ Holding " + symbol + " (" + str(round(hold_mins, 1)) + "m) — min hold not reached")
...
def run_bot():
    ...
    for analysis in all_sells:
        ...
        # Only signal-sell if in profit — never signal-sell at a loss
        if plpc <= 0:
            log.info("⏳ Skip signal sell " + pos_sym + " — not in profit yet (" + str(round(plpc*100,2)) + "%)")
            continue
